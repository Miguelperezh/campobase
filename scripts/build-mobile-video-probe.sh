...
ffmpeg -y -i "$ROOT/original.mp4" -map 0:v:0 -c:v libx264 -profile:v main -level:v 4.0 -pix_fmt yuv420p -preset medium -crf 23 -movflags +faststart -an "$ROOT/transcoded-mobile.mp4"
echo "=== ffprobe ==="
