...
test('la versión 2.20.0 está sincronizada en paquete, lock y caché PWA', async () => {
  ...
  assert.match(sw, /campobase-v2\.20\.0/);
});
...
test('la limpieza elimina los ejercicios precargados malos y el builder de sesión es intuitivo', async () => {
  ...
  assert.match(sw, /campobase-v2\.20\.0/, 'caché actualizada');
});
