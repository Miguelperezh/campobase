...
test('la versión 2.34.0 está sincronizada en paquete, lock y caché PWA', async () => {
  ...
  assert.equal(pkg.version, '2.34.0');
  assert.equal(lock.version, '2.34.0');
  assert.equal(lock.packages[''].version, '2.34.0');
  assert.match(sw, /campobase-v2\.34\.0/);
});
...
test('la limpieza elimina los ejercicios precargados malos y el builder de sesión es intuitivo', async () => {
  ...
  assert.match(sw, /campobase-v2\.34\.0/, 'caché actualizada');
});
