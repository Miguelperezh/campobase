...
test('la versión 2.32.1 está sincronizada en paquete, lock y caché PWA', async () => {
  ...
  assert.equal(pkg.version, '2.32.1');
  assert.equal(lock.version, '2.32.1');
  assert.equal(lock.packages[''].version, '2.32.1');
  assert.match(sw, /campobase-v2\.32\.1/);
});
...
test('la limpieza elimina los ejercicios precargados malos y el builder de sesión es intuitivo', async () => {
  ...
  assert.match(sw, /campobase-v2\.32\.1/, 'caché actualizada');
});
