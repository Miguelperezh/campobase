...
test('la versión 2.32.2 está sincronizada en paquete, lock y caché PWA', async () => {
  ...
  assert.equal(pkg.version, '2.32.2');
  assert.equal(lock.version, '2.32.2');
  assert.equal(lock.packages[''].version, '2.32.2');
  assert.match(sw, /campobase-v2\.32\.2/);
});
...
test('la limpieza elimina los ejercicios precargados malos y el builder de sesión es intuitivo', async () => {
  ...
  assert.match(sw, /campobase-v2\.32\.2/, 'caché actualizada');
});
