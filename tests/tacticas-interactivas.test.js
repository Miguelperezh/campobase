...
test('el catálogo del manual expone tres tácticas maestras (1-3-2-1, 1-2-3-1 y 1-2-1-3)', () => {
  assert.equal(TACTICAS_INTERACTIVAS.length, 3);
  const [t1321, t1231, t1213] = TACTICAS_INTERACTIVAS;
...
test('findTacticaInteractiva y tacticasDeFormacion siguen funcionando', () => {
  ...
  assert.equal(tacticasDeFormacion('1-2-2-2').length, 0);
});
