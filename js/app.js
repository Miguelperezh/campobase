...
async function init() {
  ...
  if ('serviceWorker' in navigator && !isValidationPreview) {
    if (isLocal) {
      ...
      navigator.serviceWorker.register('./sw.js?v=20260925-v61-equipo-nav-persistence').then((reg) => {
        reg.update().catch(() => {});
      }).catch(handleError);
    }
  }
