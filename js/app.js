...
async function init() {
  ...
  if ('serviceWorker' in navigator && !isValidationPreview) {
    if (isLocal) {
      ...
      navigator.serviceWorker.register('./sw.js?v=20260925-v60-equipo-prep-persistence').then((reg) => {
        reg.update().catch(() => {});
      }).catch(handleError);
    }
  }
